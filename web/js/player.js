...
/** Audio index and audio folder, resolved against this module rather
    than the page, so it does not matter which page imported it. The
    same trick api.js uses for the catalogue snapshot. */
const INDEX_URL = new URL('../data/audio-index.json', import.meta.url).href;
const AUDIO_BASE = new URL('../data/audio/', import.meta.url).href;
...
function srcFor(track) {
  ...
  return rid ? AUDIO_BASE + encodeURIComponent(byId.get(rid)) : null;
}
